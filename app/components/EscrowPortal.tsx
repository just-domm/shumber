...
const EscrowPortal: React.FC<EscrowPortalProps> = ({ item, onRelease }) => {
        ...
        {step === 'INITIAL' && (
          <>
            ...
            <button 
              ...
              className="w-full bg-black text-white font-bold py-4 rounded-2xl flex items-center justify-center space-x-3 hover:scale-[1.02] transition-transform"
            >
        ...
        {step === 'SCANNING' && (
          <div className="space-y-8 py-10">
            <div className="w-56 h-56 border-4 border-black border-dashed rounded-3xl mx-auto flex items-center justify-center relative overflow-hidden bg-gray-50">
               ...
               <p className="text-[10px] font-black text-gray-300 tracking-widest">CAMERA ACTIVE</p>
            </div>
        ...
        {step === 'CONFIRMED' && (
          <>
            ...
            <div>
              ...
              <p className="text-gray-500 mt-2 text-sm">Quality matches Gemini score of {item.qualityScore}%. Authorized to release KES {item.currentBid * item.quantity}.</p>
            </div>
            <button 
              onClick={onRelease}
              className="w-full bg-green-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-green-100 hover:bg-green-700 transition-colors"
            >
